...
/* harmony import */ var _modules_telmask_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./modules/telmask.js */ "./src/js/modules/telmask.js");
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const iconMenu = document.querySelector('.header__icon'),
    menuBody = document.querySelector('.header__inner');
  const burgerMenu = () => {
    if (iconMenu && menuBody) {
      iconMenu.addEventListener('click', function () {
        let scrollWidth = scrollbarWidth() + 'px';
        if (!document.body.classList.contains('_lock')) {
          document.body.setAttribute('style', 'margin-right: ' + scrollWidth + ';');
        } else {
          document.body.setAttribute('style', 'margin-right: 0px;');
        }
  ...
  const blockOne = document.querySelector('.main__image'),
    targetOne = document.querySelector('.main__links-block'),
    homeOne = document.querySelector('.main__inner');
  const blockTwo = document.querySelector('.offer__video'),
    targetTwo = document.querySelector('.offer__text'),
    homeTwo = document.querySelector('.offer__inner');
  if (window.innerWidth <= 768) {
    targetOne.before(blockOne);
    targetTwo.before(blockTwo);
  }
  window.addEventListener('resize', function () {
    if (window.innerWidth <= 768) {
      ...
      homeOne.append(blockOne);
      homeTwo.append(blockTwo);
    }
  });
  ...
  const slider = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"]('.like__slider', {
    slidesPerView: 2,
    slidesPerGroup: 1,
    spaceBetween: 30,
    autoHeight: true,
  ...
  //===Set active button for slider=============
  const btns = document.querySelectorAll('.like__button');
  btns.forEach(btn => {
  ...
  const phoneNumber = document.querySelector("#phone"),
    buttonSubmit = document.querySelector('.request__button'),
    form = document.querySelector('.request__form');
  const iti = intl_tel_input__WEBPACK_IMPORTED_MODULE_2__(phoneNumber, {
    initialCountry: 'ua',
  ...
  buttonSubmit.addEventListener('click', e => {
    ...
    phoneNumber.value = code + val;
    form.submit();
  });
  (0,_modules_telmask_js__WEBPACK_IMPORTED_MODULE_1__["default"])('#phone');
  ...
  if (menuLinks.length > 0) {
    ...
    function onMenuLinkClick(e) {
      ...
      if (!menuLink.dataset.goto) {
        menuLink = e.target.closest('[data-goto]');
      }
      let corr;
      if (window.innerWidth <= 375) {
        corr = document.querySelector('header').offsetHeight / 3;
      } else {
        corr = document.querySelector('header').offsetHeight * 2 / 3;
      }
      if (menuLink.dataset.goto && document.querySelector(menuLink.dataset.goto)) {
        const gotoBlock = document.querySelector(menuLink.dataset.goto);
        const gotoBlockValue = gotoBlock.getBoundingClientRect().top - corr + window.scrollY - document.querySelector('header').offsetHeight;
